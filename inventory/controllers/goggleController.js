...
exports.goggle_update_get = (req, res, next) => {
  async.parallel(
    {
      goggle(callback) {
        Goggle.findById(req.params.id)
    ...
    (err, results) => {
      ...
      if (results.goggle == null) {
        const error = new Error("Goggle model not found in database")
        error.status = 404
        return next(error)
      }
      res.render("goggle_form", {
...
exports.goggle_update_post = [
  ...
  (req, res, next) => {
    ...
    Goggle.findByIdAndUpdate(req.params.id, goggle, (err) => {
      if (err) {
        return next(err)
      }
      res.redirect(goggle.url)
